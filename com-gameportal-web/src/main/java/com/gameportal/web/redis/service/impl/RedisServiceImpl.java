package com.gameportal.web.redis.service.impl;
...
import com.gameportal.web.redis.service.IRedisClientResMgr;
...
public class RedisServiceImpl implements IRedisService {
	...
	public ShardedJedis getShardedJedis() {
		try {
			...
			logger.error("<|>2013011801<|><|>数据缓存工具类<|>getShardedJedis链接<|><|><|>异常<|><|>备用参数<|>",e);
			return null;
		}
	}
...
//			logger.info("<|>2013011813<|>数据缓存工具类<|>addToRedis把对象写入redis缓存中<|>"
//					+ key + "/" + objClass + "<|><|><|>备用参数<|>");
			return (T) obj;
		} catch (Exception e) {
			logger.error("<|>2013011813<|>数据缓存工具类<|>addToRedis把对象写入redis缓存中<|>"
					+ key + "/" + objClass + "<|><|><|>备用参数<|>", e);
			return null;
		}
	}
...
//			logger.info("将数据："+value+"插入到list中前面");
			if(time > 0) shardedJedis.expire(key, time);
			return res;
		} else {
			Long res = shardedJedis.linsert(key, LIST_POSITION.AFTER, pivot, value);
			if(time > 0) shardedJedis.expire(key, time);
			logger.info("将数据："+value+"插入到list中后面");
			return res;
		}
	}
