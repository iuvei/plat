  ...
  <tr>
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <form id="ptClearingFrm" name="ptClearingFrm" action="${action_path}manage/helpxima/queryAGMemberXima.do" method="post">
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-bottom:15px;">
          <tr>
            ...
            <td valign="middle" class="regname">结算平台：
				<#if gamePlatform??>
					${gamePlatform}
				</#if>
            </td>
             ...
             <#if betLogTotal??>
	             <#list betLogTotal as bet>
		            <tr onmouseover="style.backgroundColor='#f4f4f4'" onmouseout="style.backgroundColor=''" bgcolor="#e4e4e4">
		              <td height="30" align="center" valign="middle"><input type="checkbox" name="checkVal" value="${bet.clearingstatus}"/><input type="hidden" name="accounts" value="${bet.account}"/></td>
...
!function($){
	$(function(){
		...
		$("#ptclearing_btn").click(function(){
			var platgames="";
			$("input[name='clearnPlat']:checked").each(function(){
				var gc = $(this).val();
				var gcsub = gc.substring(gc.indexOf('#')+1,gc.length);
      			platgames+=gcsub;
      			platgames+=",";               
    		}); 
    		$("#platformcode").val(platgames.substring(0,platgames.length-1));
...
function fenye(pageNo){
    ...
    var platgms=platgames.substring(0,platgames.length-1);
	var url = "${action_path}manage/helpxima/queryAGMemberXima.do?startDate="+startDate+"&platformcode="+platgms+"&pageNo="+pageNo;
...
function ximaFun(){
    ...
    var itcIds=itcId.substring(0,itcId.length-1);
	$.ajax({
		type : "POST",
