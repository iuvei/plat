...
{
	function privinceSelect(cmb, str)
	{	
		
		if(typeof str == "undefined" || str == ""){
			str = $(cmb).children().eq(0).text();
			initProvinceObj(str);
			$(cmb).closest(".ui-ddl").find(".input").text(str);
			return;
		}
		initProvinceObj(str);
		for(var i=0; i<$(cmb).children().length; i++)
		{
			if($(cmb).children().eq(i).text() == str)
			{
				$(cmb).closest(".ui-ddl").find(".input").text(str);
				return;
			}
	...
	function initProvinceObj(provinceName){
	...
	function initCityObj(cityName){
		if(typeof cityName == "undefined" || cityName == ""){
			return ;
		}
		for(var i=0; i<provinceObj.cityList.length; i++)
		{
			if(provinceObj.cityList[i].name == cityName){
				cityObj = provinceObj.cityList[i];
				break;
			}
		}
	}
	...
	function changeProvince()
	{
		...
		var provinceName = $(this).closest(".ui-ddl").find(".input").text();
		initProvinceObj(provinceName);
		$(cmbCity).empty();
		for(var i=0; i<provinceObj.cityList.length; i++)
		{
			cmbAddOption(cmbCity, provinceObj.cityList[i].name);
		}
		$(cmbCity).closest(".ui-ddl").find(".input").text(provinceObj.cityList[0].name);
	}
	function initCity(cmbCity,defaultCity)
	{
		$(cmbCity).empty();
		for(var i=0; i<provinceObj.cityList.length; i++)
		{
		...
		citySelect(cmbCity, defaultCity);
	}
	...
	for(var i=0; i<provinceList.length; i++)
	{
		cmbAddOption(cmbProvince, provinceList[i].name);
	}
	privinceSelect(cmbProvince, defaultProvince);
	initCity(cmbCity);
