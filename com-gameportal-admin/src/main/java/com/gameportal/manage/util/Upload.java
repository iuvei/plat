...
public class Upload {
	...
	public boolean saveUpload(InputStream stream,String filename,String path,String firstPath){
		...
		logger.info("文件上传根目录："+path);
		int fileMaxSize = 4;
		//创建一个基于磁盘的文件工厂
		DiskFileItemFactory factory = new DiskFileItemFactory();
		factory.setSizeThreshold(4096);//设置缓冲区大小，这里是4kb
		factory.setRepository(new File(path+WebConstants.ROOT_PATH+WebConstants.TEMP_FIRST_PATH));//设置缓冲区目录
		ServletFileUpload upload = new ServletFileUpload();
		upload.setSizeMax(fileMaxSize * 1000000); // 设置最大文件尺寸，这里是4MB
		String paths = path+WebConstants.ROOT_PATH+firstPath;
		logger.info("文件上传子目录："+paths);
		//检查文件是否存在，存在则删除
        ...
        try {
			...
			logger.info("限制文件大小："+upload.getSizeMax());
			OutputStream op = new FileOutputStream(file);  
		    bStream.writeTo(op);
		    bStream.flush();
			bStream.close();
