...
Ext.ns("Ext.market.field"); // 自定义一个命名空间
field = Ext.market.field; // 定义命名空间的别名
field = {
	all : '/manage/system/field/querySystemField.do',// 加载所有
	save : "/manage/system/field/saveSystemField.do",//保存
	del : "/manage/system/field/delSystemField/",//删除
	ENABLED : eval('(${fields.enabled})'),
	pagesizes:eval('(${fields.pagesize})'),
...
field.tbar = [field.addAction, '-', field.editAction, '-',
		field.deleteAction, '-', field.searchField,'-'];
/** 底部工具条 */
...
/** 基本信息-表格 */
field.grid = new Ext.grid.EditorGridPanel({
			store : field.store,
...
field.delFun = function() {
	...
	Ext.Msg.confirm('提示', '确定要删除这条记录吗?', function(btn, text) {
				if (btn == 'yes') {
					// 发送请求
					Share.AjaxRequest({
								url : field.del + record.data.fieldId +".do",
								callback : function(json) {
									field.alwaysFun();
									field.store.reload();
								}
