...
Ext.ns("Ext.market.role"); // 自定义一个命名空间
role = Ext.market.role; // 定义命名空间的别名
role = {
	all : '/manage/system/role/queryRole.do',// 加载所有
	save : "/manage/system/role/saveSystemRole.do",//保存
	del : "/role/del/",//删除
	allModules : "/manage/system/module/queryModulesAll.do",//系统菜单
	saveModules : "/manage/system/role/saveRoleModule.do",//保存角色与系统菜单
	myModules : "/manage/system/module/queryModulesByRoleId/",
	childNodes : '',
	pageSize : 20
	// 每页显示的记录数
};
...
role.store = new Ext.data.Store({
			...
			}, ['roleId', 'roleName', 'roleDesc', 'status']),
			listeners : {
			...
			listeners : {
				'cellclick' : function(obj, rowIndex, columnIndex, e) {
					...
					if (record) {
						...
						// 当前选择用户的资源信息
						Share.AjaxRequest({
									url : role.myModules + record.data.roleId+".do",
									callback : function(json) {
										role.treePanel.getRootNode().expand(true,true);//默认全部展开
										// 勾选资源树的checkbox
										if (json.length > 0) {
											for (var i = 0; i < json.length; i++) {
												moduleId = json[i].moduleId;
												var treeNode = role.treePanel.getNodeById("_generate_" + moduleId);
	...
	if (treeNode.hasChildNodes()) {
		for (i = 0; i < treeNode.childNodes.length; i++) {
			if (treeNode.childNodes[i].getUI().checkbox) {
				if (treeNode.childNodes[i].getUI().checkbox.checked) {
					// 去除前缀
					role.childNodes += treeNode.childNodes[i].id.replace("_generate_", "") + ',';
				}
			}
		}
		for (i = 0; i < treeNode.childNodes.length; i++) {
