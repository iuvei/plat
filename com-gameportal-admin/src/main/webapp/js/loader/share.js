 ...
 * 结算状态
 */
Share.flag = function(v){
	if(v == '1'){
		return '<span style="color:green;">可洗码</span>';
	}else if(v == '0'){
		return '<span style="color:red;">不洗码</span>';
	}else if(v == '-1'){
		return '<span style="color:red;">重置试玩额度</span>';
	}else if(v == '-2'){
		return '<span style="color:red;">注单被篡改</span>';
	}else if(v == '-8'){
		return '<span style="color:red;">此局注单被取消</span>';
	}else if(v == '-9'){
		return '<span style="color:red;">取消注单</span>';
	}
	return v;
}
/**
 * 保留两位小数
 */
Share.decimalTwo = function(value){
