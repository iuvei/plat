$(document).ready(function () {
    Ext.ns("Ext.market.login"); // 自定义一个命名空间
    login = Ext.market.login; // 定义命名空间的别名
    login = {
        login: "/login.do",
        main: ctx + "/manage/index.do",
        findpwd: ctx + "/findpwd",
    ...
    // 状态栏HTMl
    login.bbarHtml = '本机物理地址：<span id="thisMac"></span>';
    //	if ('true' == 'true') {
    ...
    login.loginWindow = new Ext.Window({
        renderTo: Ext.getBody(),//'login-win-div',
        id: 'loginWindow',
        title: '用户登录',
        width: 300,
        height: 180,
    ...
    login.loginFun = function () {
        ...
        if (account == "" || password == "") {
            ...
            // 判断cookie中的密码
            password = cookiePassword == password ? password : password;
            // 发送请求
            Share.AjaxRequest({
                url: login.login,
                params: {
	...
	//$("#thisMac").html(activeXDemo.getMac());
});