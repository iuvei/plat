 ...
 */
@RestController
@RequestMapping("/roomYLReport")
@Auth("RoomYLReport")
public class RoomYLReportAction {

    @Autowired
    private IRoomYLService roomYLService;
    ...
    public NaResponse<RoomReportRequest> search(@RequestBody RoomReportRequest reportRequest){
		...
		if(reportRequest.getStatisType()==RoomStaEnum.TEAMSTA.get()){ //下级团队维度
			return NaResponse.createSuccess(roomYLService.queryRoomYLTeamReport(reportRequest));
		}else if(reportRequest.getStatisType()==RoomStaEnum.PROXYSTA.get()){ //代理汇总维度
			return NaResponse.createSuccess(roomYLService.queryRoomYLProxyReport(reportRequest));
		}else if(reportRequest.getStatisType()==RoomStaEnum.ROOMSTA.get()){ //房间汇总维度
			return NaResponse.createSuccess(roomYLService.queryRoomYLRoomReport(reportRequest));
		}else{ //单局汇总维度
			return NaResponse.createSuccess(roomYLService.queryRoomYLRoundReport(reportRequest));
		}
    }
}
