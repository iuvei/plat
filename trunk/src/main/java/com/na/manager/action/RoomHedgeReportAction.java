 ...
 */
@RestController
@RequestMapping("/roomHedgeReport")
@Auth("RoomHedgeReport")
public class RoomHedgeReportAction {
    @Autowired
    private IRoomHedgeService roomHedgeService;
    ...
    public NaResponse<RoomReportRequest> search(@RequestBody RoomReportRequest reportRequest){
		...
		if(reportRequest.getStatisType()==RoomStaEnum.TEAMSTA.get()){ //下级团队维度
			return NaResponse.createSuccess(roomHedgeService.queryRoomHedgeTeamReport(reportRequest));
		}else if(reportRequest.getStatisType()==RoomStaEnum.PROXYSTA.get()){ //代理汇总维度
			return NaResponse.createSuccess(roomHedgeService.queryRoomHedgeProxyReport(reportRequest));
		}else if(reportRequest.getStatisType()==RoomStaEnum.ROOMSTA.get()){ //房间汇总维度
			return NaResponse.createSuccess(roomHedgeService.queryRoomHedgeRoomReport(reportRequest));
		}else{ //单局汇总维度
			return NaResponse.createSuccess(roomHedgeService.queryRoomHedgeRoundReport(reportRequest));
		}
    }
   
	
	@PostMapping("/view")
	public NaResponse<BetOrderReportRequest> view(@RequestBody BetOrderReportRequest reportRequest){
		reportRequest.setPlayId(1);
