...
import {RoomHedgeTableComponent} from "./roomhedge-table.component";
import {RoomHedgeService} from "./roomhedge.service";
import {ModalDirective} from "ng2-bootstrap";
import {TransRecordService} from "../transrecord/transrecord.service";

@Component({
  templateUrl: 'roomhedge.component.html',
  styleUrls: ['./roomhedge.component.scss']
})
export class RoomHedgeComponent implements  OnInit,OnDestroy {
  private tableList: any[] ;
  private searchModel : SearchModel = new SearchModel();
  public tableindex : number = 0;
  private realTableList: any[] ;

  @ViewChild('roomHedgePanel', {read: ViewContainerRef})
  private roomHedgePanel: ViewContainerRef;

  @ViewChild('betOrderDialog')
  public betOrderDialog:ModalDirective;

  private listTableComponent : ComponentRef<RoomHedgeTableComponent>[] = [];

  constructor(private roomHedgeService:RoomHedgeService,private transRecordService:TransRecordService, private _translate:TranslateService,
              private toaster:ToasterService,private _componentFactoryResolver: ComponentFactoryResolver) {
  }

  ngOnInit() {
    let obj = JSON.parse(sessionStorage.getItem("roomHedge-component"));
    if(obj){
  ...
  ngOnDestroy(): void {
    ...
    sessionStorage.setItem("roomHedge-component",JSON.stringify(data));
  }

  public search() {
    this.clearAllTable();
    this.roomHedgeService.search(this.searchModel.startTime,this.searchModel.endTime,
  ...
  searchChild(data) {
    ...
    let tableComponent :RoomHedgeTableComponent = data.component;
  ...
  private clearAllTable() {
    let comp: ComponentRef<RoomHedgeTableComponent>;
    do {
  ...
  private createTableComponent(roomStatisList:any[], parentAgent:any) {
    let componentFactory = this._componentFactoryResolver.resolveComponentFactory(RoomHedgeTableComponent);
    let componentRef = this.roomHedgePanel.createComponent(componentFactory);
