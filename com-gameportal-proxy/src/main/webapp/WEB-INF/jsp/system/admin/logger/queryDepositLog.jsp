	...
	<body>
	 ...
	 <div class="container-fluid" id="main-container">
			...
			<div id="main-content" class="clearfix">
					...
					<div id="page-content" class="clearfix">
						<div class="page-header position-relative" style="padding-bottom:0px;">
							<form class="form-inline" action="${ctx}userproxy/queryDepositLog.do" method="post" name="depositlogsFrm">
								<span class="input-icon">
									<label>会员账号：</label>
									<input type="text" name="account" class="form-control" style="width: 80px;" value="${pd.uaccount}">
								</span>
								<span class="input-icon">
									<label>入款类型：</label>
									<select class="form-control" name="paymethods" style="border-radius: 0px;width:120px;">
									<option value="">全部</option>
									<c:if test="${pd.paymethods=='' || pd.paymethods==null}">
										<option value="1">公司入款</option>
										<option value="2">在线支付</option>	
									</c:if>
									<c:if test="${pd.paymethods=='0'}">
										<option value="1" selected>公司入款</option>
										<option value="2">在线支付</option>	
									</c:if>
									<c:if test="${pd.paymethods=='1'}">
										<option value="1">公司入款</option>
										<option value="2" selected>在线支付</option>	
									</c:if>
					  			</select>
								</span>
								<span class="input-icon">
									<label>交易类型：</label>
									<select class="form-control" name="paytyple" style="border-radius: 0px;width:120px;">
										<option value="">全部</option>
	 									<c:if test="${pd.paytypleSet=='' || pd.paytypleSet==null}">
											<option value="1">充值</option>
											<option value="2">赠送</option>	
										</c:if>
										<c:if test="${pd.paytypleSet=='0'}">
											<option value="1" selected>充值</option>
											<option value="2">赠送</option>	
										</c:if>
										<c:if test="${pd.paytypleSet=='2'}">
											<option value="1">充值</option>
								...
								<span class="input-icon">
									...
									<button type="submit" class="btn btn-purple btn-small _querydepositlogsFrm">搜索<i class="icon-search icon-on-right"></i></button>
								</span>
							</form>
						</div>
								...
								<table id="table_paylog" class="table table-striped table-bordered table-hover">
									...
									<tbody class="_orderList">
										<c:if test="${not empty orderList}">
											<c:forEach items="${orderList }" var="order">
												<tr>
													...
													<c:if test="${order.paytyple == 0}">
														<th class="text-danger">充值</th>
													</c:if>
													<c:if test="${order.paytyple == 2}">
														<th class="text-warning">赠送</th>
													</c:if>
													<c:choose>
														<c:when  test="${order.paymethods == 0}"><th class="text-danger">公司入款</th></c:when >
														<c:when  test="${order.paymethods == 1}"><th class="text-danger">在线支付</th></c:when >
														<c:otherwise><th class="text-danger">赠送</th></c:otherwise>
													</c:choose>
										...
										<c:if test="${empty orderList}">
											<tr><td colspan="8" class="center">暂无记录。</td></tr>
										</c:if>
									</tbody>
								</table>
		...
		$(function(){
			...
			$("._querydepositlogsFrm").click(function(){
				depositlogsFrm.submit();
			});
			...
			test("page", function(){
		        ...
		        var options = {
		            ...
		            onPageClicked: function (event, originalEvent, type, page) {
		            	...
		            	$("._orderList").html("<tr><td colspan='8' class='center'><img src='/static/images/loadingi.gif'/>&nbsp;&nbsp;加载中,请稍后...</td></tr>");
		            	depositlogsFrm.submit();
		            }
		        }
			...
			$("#queryDepositLog",".nav-list").addClass("active");
		});
	</script>
	</body>
</html>