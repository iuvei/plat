...
@Component
public class QuickTable extends Table {

	private Logger log = LoggerFactory.getLogger(QuickTable.class);
				...
				public void run() {
					try {
						...
						if(betList != null && !betList.isEmpty()) {
							...
							BetOrderSourceEnum.SEAT == item.getBetOrderSource()).collect(toList());
							sideBetList = betList.stream().filter( item -> 
							BetOrderSourceEnum.SIDENOTE == item.getBetOrderSource()).collect(toList());
						} else {
							betList = null;
						}
						threadSettlement(vtableId, seatBetList,
								showResult, resultMap,
								table, BetOrderSourceEnum.SEAT, filterTradeItemIds);
						
						threadSettlement(vtableId, sideBetList,
								showResult, resultMap,
								table, BetOrderSourceEnum.SIDENOTE, filterTradeItemIds);
...
//		BaccaratCache.freshVirtualTableList(vtableList);
		...
		normalTableThreadPool.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					BetOrderSourceEnum.MANY_TYPE == item.getBetOrderSource()).collect(toList()),
					showResult, resultMap, table, 
					BetOrderSourceEnum.MANY_TYPE, filterTradeItemIds);
				} catch (Throwable t) {
					log.error(t.getMessage(),t);
				} finally {
					countDownLatch.countDown();
		...
		normalTableThreadPool.execute(new Runnable() {
			...
			public void run() {
				try {
					threadSettlement(null, berOrderList.stream().filter( item -> 
					BetOrderSourceEnum.GOOD_ROAD == item.getBetOrderSource()).collect(toList()),
					showResult, resultMap, table, BetOrderSourceEnum.GOOD_ROAD, filterTradeItemIds);
				} catch (Throwable t) {
					log.error(t.getMessage(),t);
				} finally {
					countDownLatch.countDown();
	 ...
	 * @param sourceEnum       来源
	 */
	private void threadSettlement(final Integer vtableId, List<BetOrder> berOrderList,
			String showResult, Map<PlayEnum,AbstractPlayRule> resultMap,
			GameTable table, final BetOrderSourceEnum sourceEnum,
		...
		if(berOrderList != null && !berOrderList.isEmpty()) {
			berOrderList.stream().forEach( item -> {
		...
		if(BetOrderSourceEnum.SEAT == sourceEnum || BetOrderSourceEnum.SIDENOTE == sourceEnum) {
	...
	public void setCard(SetCardPara params, SocketIOClient client,
		...
		if (params.getCardNum() != null && params.getCardType() != null && params.getIndex() != null) {
			...
			synchronized(this) {
				...
				if((roundExtPO.getBankCard1Number() != null && roundExtPO.getBankCard2Number() != null
...
//				handOutCardOneByOne(flag, table, roundExtPO, client, betPlayerMap);
	...
	/**
	 * 分发卡片  让用户咪牌
	 * @param type   1表示闲前2张 2庄闲前2张  3表示闲第3张  4表示庄第3张
	 * @param table
	 * @param roundExtPO
	 * @param dealerClient
	 */
	private void handOutCard(Integer type, GameTable table, RoundExtPO roundExtPO,  SocketIOClient dealerClient , Map<TradeItemEnum,Set<User>> betPlayerMap) {
		...
		switch (type) {
			case 1:
				...
				cardList.add(cardInfo);
				handOutUser(TradeItemEnum.PLAYER, table, betPlayerMap, 
						cardList, playTaskCardList, countDownSecond * 2, 
						dealerClient, dealerCardList, playTaskNoticeUserList, delaySecond);
				if(dealerCardList != null && dealerCardList.size() > 0) {
					noticeDealer(dealerClient, dealerCardList, table);
				}
			...
			case 2:
				...
				cardList.add(cardInfo);
				handOutUser(TradeItemEnum.BANKER, table, betPlayerMap, 
						cardList, bankTaskCardList, countDownSecond * 2, 
						dealerClient, dealerCardList, bankTaskNoticeUserList, delaySecond);
...
//			taskNoticeUserList.addAll(miUserList);
	...
	private void noticeNotMiCardUser(Integer tableId, boolean isFanPai, List<UserCardResponse.CardInfo> cardList,
			Set<User> userList, int countDownSecond, Integer delaySecond) {
		//通知用户开牌
