...
public class CHPayController extends BasePayController {
	private static final Logger logger = Logger.getLogger(CHPayController.class);
	...
	public String prePayment(@PathVariable String type, @RequestParam(value = "hd") String hd,
		...
		if (StringUtils.isNotBlank(vuid)) {
			...
			if (userInfo != null) {
				...
				params.put("hd", hd);
				params.put("pgBankCode", bankCode);
				params.put("paFrpId", "OnlinePay");
				...
				logger.info("畅汇网银请求参数："+JSONObject.fromObject(pay));
				request.setAttribute("pay", pay);
				request.setAttribute("submitUrl", payPlat.getDomainname());
				return "/payment/chPayJump";
			}
		}
		return null;
	}
