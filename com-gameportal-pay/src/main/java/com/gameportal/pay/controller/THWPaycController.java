...
public class THWPaycController extends BasePayController {
	private static final Logger logger = Logger.getLogger(THWPaycController.class);
	...
	public void callBack(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		try {
			...
			PayPlatform payPlat = payPlatformService.queryPayPlatform(6L);
			...
			logger.info("充值金额：" + orderAmount + "通汇扫码后台回调信息：" + thizSign);
			if (thizSign.equalsIgnoreCase(sign)) {
				...
				if (order == null) {
					...
					if (!numberFormat.format(order.getAmount()).equals(orderAmount)) {
						...
						logger.info("通汇扫码支付充值加款成功->平台订单编号：" + order.getPlatformorders()+"金额："+order.getAmount());
						out.write("success");
					} else {
			...
			logger.error("通汇扫码后台回调失败。" + e.getMessage());
			out.write("fail");
		}
	}

	@RequestMapping("/wxth/view")
	public String view(HttpServletRequest req, HttpServletResponse resp) {
		try {
			...
			kvs.add(new KeyValue("trade_status", tradeStatus));
			String thizSign = kvs.sign(payPlat.getPlatformkey(), "UTF-8");
			logger.info("充值金额：" + orderAmount + "通汇扫码支付后台回调信息：" + thizSign);
			if (thizSign.equalsIgnoreCase(sign)) {
				PayOrder order = payPlatformService.queryPayOrderId(orderNo, null, null, "0");
